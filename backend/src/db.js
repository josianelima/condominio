...
async function createDocument(data) {
    const collection = await getMongoCollection("Condominio", "users")
...
async function createSession(data) {
    const collection = await getMongoCollection("Condominio", "sessions")
    const result = await collection.insertOne(data)
    console.log(result)
    return result
}
...
async function addCondominio(elem, data) {
    const collection = await getMongoCollection("Condominio", "users")
    const result = await collection.updateOne(
        elem, {$push: data}
    )
...
module.exports = { connectToMongo, createDocument, createSession, getCollection, findDocumentById, findDocumentByEmail, findSessionByEmail, findSessionByToken, deleteSessionByEmail, deleteDocumentById, updateDoc, addCondominio }